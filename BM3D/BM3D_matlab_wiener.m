function [imout] = BM3D_matlab_wiener(im,imout,sigma)
% sigma is the standard deviation of noise when the image intensity ranges
% from 0 to 255. It is 255 times the deviation of noise when the image 
...
%% Block Matching Procedure.
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
 %% The Third Dimension Forward Transform
 for j=1:N1
    for u=1:N1
        d3_z=zeros(N2,1); 
        d3_y=zeros(N2,1); 
        ...
        d3_z = hadper_trans_single_den{h}*d3_z; %% The 3rd dimension forward transform.
        d3_y = hadper_trans_single_den{h}*d3_y;

        group1{l}(j,u,:) = d3_z; % essentially doing 3D transform on the group
        group2{l}(j,u,:) = d3_y;

    end
 end
 Wsxr = (group2{l}.*group2{l})./((group2{l}.*group2{l})+(sigma/255)^2); % empircal Wiener shrinkage coefficients
 group1{l} = group1{l} .* Wsxr; % perform Wiener shrinkage
 ...
 %% Computing Weight Value in Each Group.
    ...
    weight(l) = 1/(norm(Wsxr,"fro")^2 * (sigma/255)^2);    
 ...
 %% The Third Dimension Inverse Transform.  
  for j=1:N1
    for u=1:N1
        d3_z=zeros(N2,1); 
        d3_z(:,1) = group1{l}(j,u,:);
        d3_z = inverse_hadper_trans_single_den{h}*d3_z;
        group1{l}(j,u,:) = d3_z(:,1); % 3D inverse transform 
    end
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
              l=l+1;  %% proceed to the next reference block
