 ...
 for a = 41:Nstep:M1+40                              %% Block matching 3D transform image denoising main function
    for b = 41:Nstep:M2+40
       ...
       im_n=im_p(a:a+N1-1,b:b+N1-1);           %% 1st image block,i.e.,reference image block.
                   ...
                   % stores the attributes of the reference block.
...
% traverse in a circular manner:
%               x(1)y(1)  x(2)y(2)  x(3)y(3)
%               x(8)y(8)     a,b    x(4)y(4)
%               x(7)y(7)  x(6)y(6)  x(5)y(5)
% uptil x(Ns*Ns-)y(Ns*Ns-1)
                Q = 1;
...
%% Block Matching Procedure.
             ...
             for R=1:length(x)
               ux = im_p(x(R):x(R)+N1-1,y(R):y(R)+N1-1);   %% Choosing blocks in the neighborhood to be matched.
                ...
                % unlike the original BM3D implementation, this
                % implementation does not use coarse prefiltering in stage
                % one.
              ...
              for R=1:N2-1                                 %% Selecting 15 most similar blocks to the reference block and store them to the group as the second block to 16th block.
                   ...
                   % store the attributes of a block.
...
for r=1:C
      if Coord(r,1)==0 % iterate until all groups have been touched
         break;
      end
      %% Group aggregation
...
% crop to M1-1 by M2-1 so that imr has the same size as the input image.
